 ...
 *******************************/
add_action('wp_head', 'pd_pcf_support_form_ajax_header');

if( !function_exists('pd_pcf_support_form_ajax_header') )
{
	function pd_pcf_support_form_ajax_header() 
	{
	...
	} //End of pd_pcf_support_form_ajax_header
 ...
 *******************************/
add_action( 'wp_ajax_process_pd_pcf_promo_form', 'process_pd_pcf_promo_form' );

if( !function_exists('process_pd_pcf_promo_form') )
{
	function process_pd_pcf_promo_form()
	{
 ...
 *******************************/
if( !class_exists('WBELPCFSupportPage') ){


	class WBELPCFSupportPage{
		...
		function __construct( $plugin_slug = null )
		{
			...
			//add_action( 'wp_ajax_process_pd_pcf_promo_form', array(&$this,'process_pd_pcf_promo_form') );
		...
		{                            
		// echo $hook; die();     
		   if( $hook == 'post-and-category-filter-for-elementor_page_wb-page-pd_pcf-support' ){	   	
		...
		function show_promo_page_callback_func()
		{
			add_submenu_page(
				...
				'manage_options',
				$this->page_slug,
				array(&$this, 'pd_pcfpromo_support_page_callback_func' )
			);
		 ...
		 *******************************/
		function pd_pcfpromo_support_page_callback_func()
		{
			
			?>
				
				<div class="pd_pcf-support-page-wrapper">
			...
			<?php
		} //End of pd_pcfpromo_support_page_callback_function
		
		
	
	} //End of the class WBELPCFSupportPage
...
//Supply Unique Promo Page Slug as the constructor parameter of the class WBELPCFSupportPage. ex: sld-page-2124a to the constructor

//Please create an unique instance for your use, example: $instance_sldf2

$instance = new WBELPCFSupportPage('pd_pcf-support');

if( is_admin() )
{
	$instance->plugin_menu_slug = "pd-post-category-filter"; //Edit Value
